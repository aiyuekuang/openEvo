...
export class GatewayManager extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // 强制使用我们自己的 Gateway 进程，避免外部进程重启导致任务中断
      // TODO: 后续可以优化为检测并复用稳定的 Gateway 进程
      logger.debug('Starting our own Gateway process (bypassing existing check)...');
  ...
  private async startProcess(): Promise<void> {
    ...
    const gatewayArgs = ['gateway', '--port', String(this.status.port), '--token', gatewayToken, '--dev'];
  ...
  private handleProtocolEvent(event: string, payload: unknown): void {
    ...
    switch (event) {
      case 'tick':
        ...
        break;
      case 'chat':
        logger.debug(`[Gateway] chat event:`, payload);
        this.emit('chat:message', { message: payload });
        break;
      case 'agent':
        logger.debug(`[Gateway] agent event:`, payload);
      ...
      default:
        ...
        logger.debug(`[Gateway] unknown event [${event}]:`, payload);
        this.emit('notification', { method: event, params: payload });
    }
  }
