 ...
 * 计算技能状态
 */
export function computeSkillStatus(skillId, metadata) {
    // 检查是否已安装并启用
 ...
 * 快速获取技能状态
 */
export function getSkillStatus(skillId, metadata) {
 ...
 * 检测技能是否就绪
 */
export function isSkillReady(skillId, metadata) {
 ...
 * 批量计算技能状态
 */
export function computeSkillsStatus(skills) {
 ...
 * 统计各状态的技能数量
 */
export function countSkillsByStatus(statusMap) {
    const counts = {
        ready: 0,
        needs_config: 0,
        needs_install: 0,
