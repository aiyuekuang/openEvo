 ...
 * ```
 */
export async function runCheckScript(skillDir, scriptPath, timeout = 10000) {
 ...
 * ```
 */
export async function runActionScript(skillDir, scriptPath, timeout = 30000) {
    const absolutePath = path.resolve(skillDir, scriptPath);
    // 检查脚本是否存在
    if (!fs.existsSync(absolutePath)) {
        return {
 ...
 * 执行检测脚本模块
 */
async function executeCheckScript(scriptPath) {
    // 动态导入脚本模块
 ...
 * 执行操作脚本模块
 */
async function executeActionScript(scriptPath) {
    // 动态导入脚本模块
    const fileUrl = pathToFileURL(scriptPath).href;
    const module = (await import(fileUrl));
 ...
 * 创建检测脚本超时 Promise
 */
function createCheckTimeout(ms) {
 ...
 * 创建操作脚本超时 Promise
 */
function createActionTimeout(ms) {
    return new Promise((_, reject) => {
        setTimeout(() => {
            reject(new Error(`脚本执行超时 (${ms}ms)`));
        }, ms);
 ...
 * 防止路径遍历攻击
 */
export function isScriptPathSafe(skillDir, scriptPath) {
    const absolutePath = path.resolve(skillDir, scriptPath);
 ...
 * 获取脚本的绝对路径
 */
export function getScriptAbsolutePath(skillDir, scriptPath) {
